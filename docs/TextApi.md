...
try {
    ...
    System.err.println("Exception when calling TextApi#get");
    e.printStackTrace();
}
```
...
TextApi apiInstance = new TextApi();
TextAnalysisRequest requestp = new TextAnalysisRequest(); // TextAnalysisRequest | 
try {
    AsyncResponseTextAnalysisResponse result = apiInstance.post(requestp);
    System.out.println(result);
} catch (ApiException e) {
    System.err.println("Exception when calling TextApi#post");
    e.printStackTrace();
}
```
